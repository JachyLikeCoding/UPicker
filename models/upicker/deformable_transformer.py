...
class DeformableTransformer(nn.Module):
    ...
    def forward(self, srcs, masks, refpoint_embed, pos_embeds, tgt, attn_mask=None):
        ...
        for lvl, (src, mask, pos_embed) in enumerate(zip(srcs, masks, pos_embeds)):
            ...
            # 这里是deformable detr中提到的level embed
            if self.num_feature_levels > 1 and self.level_embed is not None:
                # pos_embed 和 level_embed 相加
        ...
        # [bs,all hw,256] 所有特征层的拼在一起，它们就是在维度1上长度不同，尺寸越大的特征层，维度1上的数量越多
        src_flatten = torch.cat(src_flatten, 1)    # bs, \sum{hxw}, c 
        mask_flatten = torch.cat(mask_flatten, 1)   # bs, \sum{hxw}
        lvl_pos_embed_flatten = torch.cat(lvl_pos_embed_flatten, 1) # bs, \sum{hxw}, c 
        # [特征层的数量，2] 存储的是高宽
        spatial_shapes = torch.as_tensor(spatial_shapes, dtype=torch.long, device=src_flatten.device)
        # 各个src层 起始的位置，第一个 spatial_shapes.new_zeros((1,0))是在起始位置填0
        ...
        # 先经过encoder处理，后两个输出值是encoder的中间层的输出
        #########################################################
        # Begin Encoder
        #########################################################
        ...
        if self.two_stage_type == 'standard':
            if self.two_stage_learn_wh:
                ...
                input_hw = None
            # output_memory 是memory经过填充INF，经过一层全连接后的结果 [bs, all hw, 256]
            ...
            # 这一行在 deformable detr中实在gen_encoder_output_proposals方法中的，这里放到了这
            ...
            # 对应于论文中图5c的static content queries
            if self.embed_init_tgt:
                tgt_ = self.tgt_embed.weight[:, None, :].repeat(1, bs, 1).transpose(0, 1)  # nq, bs, d_model
            else:
                tgt_ = tgt_undetach.detach()

            # refpoint_embed 是 prepare_for_cdn 生成的，带有噪声的gt, tgt也是prepare_for_cdn 生成的
            # 带_后缀的这两个是上面处理的，利用encoder的output_topk筛选的
            if refpoint_embed is not None:
                # cat prepare_for_cdn 生成的去噪的，加上match部分使用的
        ...
        # Begin Decoder
        #########################################################
        # references 比 hs 多一个，hs是6，references是7，
        ...
        # references: n_dec+1, bs, nq, query_dim query_dim=4就是xywh
        #########################################################
        ...
        if self.two_stage_type == 'standard':
            ...
            if self.two_stage_keep_all_tokens:
                ...
                # [1,bs,900,4] refpoint_embed_undetach 是encoder的输出经过topk选取后的
        ...
        # init_box_proposal 最最初始的参考点位，hs_enc, ref_enc两个是encoder的输出经过topk筛选的
        ...
        for layer_id, layer in enumerate(self.layers):
            ...
            if self.bbox_embed is not None:
                ...
                if self.use_detached_boxes_dec_out:
                    ...
                    # 这里是没有脱离的，这个地方的处理就是论文中的look forward TWICE
