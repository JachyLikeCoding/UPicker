...
class upicker(nn.Module):
    """ This is the Cross-Attention Detector module that performs object detection """
    def __init__(self, backbone, transformer, num_classes, num_queries, 
                    aux_loss=False,# object_embedding_loss=False, obj_embedding_head=None,
                    iter_update=False,
                    query_dim=2, 
        ...
        if two_stage_type != 'no':
            if two_stage_bbox_embed_share:
                ...
                # encoder use decoder's box head
            ...
            if two_stage_class_embed_share:
                assert dec_pred_class_embed_share and dec_pred_bbox_embed_share
                # encoder use decoder's class head
    ...
    def forward(self, samples: NestedTensor, targets:List=None):
        ...
        if self.dn_number > 0 or targets is not None:
            input_query_label, input_query_bbox, attn_mask, dn_meta = \
                prepare_for_cdn(dn_args=(samples, targets, self.dn_number, self.dn_label_noise_ratio, self.dn_box_noise_scale),
        ...
        outputs_coord_list = []
        # layer_ref_sig 进入某一层之前的Bbox，layer_bbox_embed 这一层的Bbox头，layer_hs 这一层的layer的输出
        ...
        out = {'pred_logits': outputs_class[-1], 'pred_boxes': outputs_coord_list[-1]} #[bs,900,2], [bs,900,4]
        
        # if self.object_embedding_loss:
        #     # print("---self.feature_embed(hs[-1])")
...
class SetCriterion(nn.Module):
    ...
    def loss_object_embedding(self, outputs, targets, indices, num_boxes):
        ...
        losses = {'loss_object_embedding': torch.nn.functional.l1_loss(src_features, target_features, reduction='mean')}
        return losses
    ...
    def get_loss(self, loss, outputs, targets, indices, num_boxes, **kwargs):
        # print('\n[get_loss] outputs.keys:', outputs.keys())
        # print('--loss: ',loss)
        loss_map = {
    ...
    def forward(self, outputs, targets, return_indices=False):
        ...
        if self.training and dn_meta and 'output_known_lbs_bboxes' in dn_meta:
            # single_pad bs最大的gt的数量*2（包括了正负样本的总数量）
            ...
            for loss in self.losses:
                if 'object_embedding' in loss:
                    continue
                # print('output_known_lbs_bboxes.keys:', output_known_lbs_bboxes.keys())
                kwargs = {}
                if 'labels' in loss:
                    kwargs = {'log': False}
...
@MODULE_BUILD_FUNCS.registe_with_name(module_name='upicker')
def build_upicker(args):
