...
def save_boxes(boxes, scores, img_file, im_h, out_imgname):
    # save box files
    write_name = args.output_dir + 'star/' + img_file[:-4] + out_imgname + '.star'
...
def main(args):
    ...
    model_size_MB = total_size / (1024 ** 2)  # 转换为MB
    ...
    # 格式化输出
    if total_params >= 1e9:
        total_params_str = f"{total_params / 1e9:.2f}B"  # 十亿级别
    elif total_params >= 1e6:
        total_params_str = f"{total_params / 1e6:.2f}M"  # 百万级别
    else:
        total_params_str = f"{total_params}"  # 少于百万个参数
    print(f"\nTotal parameters: {total_params_str}")

    image_folder = Path(args.image_dir)
    ...
    for img_file in image_files:
        ...
        image_tensor = transform(source_img).unsqueeze(0).to(device)        
        ...
        if len(boxes) == 0:
            print('No boxes remain after filtering.')
            continue

        # convert boxes from [0; 1] to image scales
        print('image shape: ', source_img.size)
        im_h, im_w = source_img.size
        target_sizes = torch.tensor([[im_w, im_h]]).cuda()
        ...
        if args.mask == True:
            mask_name = args.dataset_file + '/micrographs/mask/' + img_name[:-4] + '_mask.jpg'
            ...
            save_boxes(boxes_cleaned, saved_scores, img_name, im_h, out_imgname='')
        else:
            print('no post-processing.....')
            save_boxes(boxes, scores, img_name, im_h, out_imgname='')

        save_path = args.output_dir + 'image/'
        if not os.path.exists(save_path):
            os.makedirs(save_path)
        out_imgname = save_path + img_name[:-4] + '.jpg'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--image_dir', type=str, default="data/EMPIAR10077/micrographs/processed", required=True)
